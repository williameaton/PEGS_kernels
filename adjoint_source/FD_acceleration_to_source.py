...
for i_trtype in range(len(types)):
    ...
    #fpath = f'../Tohoku/forward_data_and_plots/elastic_test/anelastic/varied_hp_filter/{lp}/{trtype}/{stn}_proc.sac'
    fpath = f'./MDJ_DATA/{trtype}/{stn}_proc.sac'
...
xerf = np.linspace(-4.5, 5, len(time))
...
ax2nd.set_ylim([-3.1e-12, 2.5e-12])
...
ytick_fs = 8
axacc.set_yticks([-1e-9, 0])
axacc.set_yticklabels([-1e-9, 0], fontsize=ytick_fs)

axtap.set_yticks([0, 1])
axtap.set_yticklabels([0, 1], fontsize=ytick_fs)

ax2nd.set_yticks([-2e-12, 0, 2e-12])
ax2nd.set_yticklabels([-2e-12, 0, 2e-12], fontsize=ytick_fs)

axfin.set_yticks([-1e-12, 1e-12])
axfin.set_yticklabels([-2e-12, 2e-12], fontsize=ytick_fs)
...
np.savetxt(f'./MDJ_DATA/adjoint_source_MDJ_{channel}', X=np.array([adj_time, adj_src]))