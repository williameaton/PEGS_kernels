...
for i_trtype in range(len(types)):
    ...
    fpath = f'./{fpath_master}/{trtype}/{stn}_proc.sac'
...
# Load the forward stf for timestep checks:
stf = np.loadtxt(f"./{fpath_master}/plot_source_time_function.txt")
...
np.savetxt(f'./{fpath_master}/adjoint_source_{stn}_{channel}', X=np.array([adj_time, adj_src]))